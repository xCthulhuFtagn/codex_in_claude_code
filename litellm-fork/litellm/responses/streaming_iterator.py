...
class BaseResponsesAPIStreamingIterator:
    ...
    def __init__(
        ...
        # Some providers (e.g. ChatGPT/Codex) emit response.completed with an
        # empty output list; the real content only arrives via preceding
        # response.output_item.done / response.output_text.done events. Track
        # those here so they can backfill an empty terminal response.
    ...
    def _process_chunk(self, chunk) -> Optional[Any]:
        ...
        try:
            ...
            if isinstance(parsed_chunk, dict):
                ...
                # Accumulate output items as they stream in. Some providers (e.g.
                # ChatGPT/Codex) emit response.completed with an empty output list;
                # the real content only arrives via these preceding events, so they
                # must be tracked to backfill an empty terminal response below.
                ...
                if openai_responses_api_chunk and _chunk_type in (
                    ...
                    if _chunk_type != openai_types.ResponsesAPIStreamEvents.RESPONSE_FAILED:
                        ...
                        if response_obj_for_backfill is not None and not getattr(
                            response_obj_for_backfill, "output", None
                        ):
                            merged_items = {
